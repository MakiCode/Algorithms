...
public class KdTree {
	...
	public Point2D nearest(Point2D p) {
		...
		return nearest(root, p, root.value.distanceSquaredTo(p), 0);
	} // a nearest neighbor in the set to p; null if set is empty

	private Point2D nearest(Node node, Point2D point, double bestDistance,
			int height) {
		if (node == null) {
			return null;
		} 
		...
		double distance = node.value.distanceSquaredTo(point);
		if (height % 2 == 0) {
			if (point.x() >= node.value.x()) {
				if (distance < bestDistance) {
					return nearest(node.right, point, distance, height + 1);
				} else {
					return nearest(node.right, point, bestDistance, height + 1);
				}
			} else {
				if (distance < bestDistance) {
					return nearest(node.left, point, distance, height + 1);
				} else {
					return nearest(node.left, point, bestDistance, height + 1);
				}
			}
		} else {
